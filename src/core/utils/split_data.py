...
def split_data(data: List, test_data: List, templ_from: int = 0,
               templ_to: int = 5) -> Tuple:
    ...
    for group_num, group in enumerate(test_data):
        for img_num, img in enumerate(group):
            if templ_from > img_num or img_num >= templ_to:
                X_test.append(img)
